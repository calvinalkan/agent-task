...
`
	...
	writeErr := os.WriteFile(mockEditor, []byte(script), 0700) //nolint:gosec // needs to be executable
	if writeErr != nil {
...
func TestEditorCommand(t *testing.T) {
	...
	t.Run("config editor used first over EDITOR env", func(t *testing.T) {
		...
		exitCode := Run(nil, &createStdout, nil, []string{
			"tk", "-C", tmpDir, "create", "Test Ticket",
		}, nil)
		if exitCode != 0 {
		...
		cfgErr := os.WriteFile(cfgPath, []byte(cfgContent), 0600)
		if cfgErr != nil {
		...
		invoked, readErr := os.ReadFile(invokedFile) //nolint:gosec // test file path
		if readErr != nil {
	...
	t.Run("fallback editor when no EDITOR", func(t *testing.T) {
		...
		if editor != "zed" && editor != "vi" && editor != "nano" {
			t.Errorf("editor = %q, want 'zed', 'vi', or 'nano'", editor)
		}
	})
	...
	t.Run("no editor found error", func(t *testing.T) {
		...
		cfg := Config{TicketDir: ".tickets", Editor: "/nonexistent/editor"}
		env := []string{"EDITOR=/also/nonexistent"}
