...
//nolint:dupl // test structure similar to start_test.go but tests different command
...
func TestCloseInProgressTicket(t *testing.T) {
	...
	exitCode = Run(nil, &stdout, &stderr, []string{"tk", "-C", tmpDir, "close", ticketID}, nil)
	...
	assertContains(t, content, "status: closed")
}
...
func TestCloseAddsTimestamp(t *testing.T) {
	...
	content := readTicket(t, ticketDir, ticketID)
	assertContains(t, content, "closed: ")
...
func TestCloseAlreadyClosedTicketError(t *testing.T) {
	...
	err := writeTicketContent(t, ticketDir, ticketID, content)
	if err != nil {
		t.Fatal(err)
	}
...
func TestCloseStdoutEmptyOnError(t *testing.T) {
	...
	exitCode := Run(nil, &stdout, &stderr, []string{"tk", "-C", tmpDir, "close", "nonexistent"}, nil)
