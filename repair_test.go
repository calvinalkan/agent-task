...
func TestRepairStaleBlocker(t *testing.T) {
	...
	content, err := os.ReadFile(ticketPath) //nolint:gosec // test file
	if err != nil {
...
func TestRepairAll(t *testing.T) {
	...
	exitCode := Run(nil, &createStdout1, nil, []string{"tk", "-C", tmpDir, "create", "Ticket 1"}, nil)
	if exitCode != 0 {
	...
	exitCode = Run(nil, &createStdout2, nil, []string{"tk", "-C", tmpDir, "create", "Ticket 2"}, nil)
	if exitCode != 0 {
	...
	// Add stale blockers to both tickets
	ticketPath1 := filepath.Join(tmpDir, ".tickets", ticketID1+".md")
	content1, _ := os.ReadFile(ticketPath1) //nolint:gosec // test file
	newContent1 := strings.Replace(string(content1), "blocked-by: []", "blocked-by: [stale1]", 1)
	_ = os.WriteFile(ticketPath1, []byte(newContent1), 0o600)

	ticketPath2 := filepath.Join(tmpDir, ".tickets", ticketID2+".md")
	content2, _ := os.ReadFile(ticketPath2) //nolint:gosec // test file
	newContent2 := strings.Replace(string(content2), "blocked-by: []", "blocked-by: [stale2]", 1)
	_ = os.WriteFile(ticketPath2, []byte(newContent2), 0o600)
	...
	if !strings.Contains(output, "Removed stale blocker: stale1") {
		t.Errorf("stdout should contain 'Removed stale blocker: stale1', got: %s", output)
	}

	if !strings.Contains(output, "Removed stale blocker: stale2") {
		t.Errorf("stdout should contain 'Removed stale blocker: stale2', got: %s", output)
	}
...
func TestRepairIdempotent(t *testing.T) {
	...
	ticketPath := filepath.Join(tmpDir, ".tickets", ticketID+".md")
	content, _ := os.ReadFile(ticketPath) //nolint:gosec // test file
