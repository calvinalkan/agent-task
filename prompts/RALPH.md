...
3. When `make test` and `make lint` passes, run all fuzz tests for at least 10 seconds each with `make fuzz-slotcache`. If all good, then update @IMPLEMENTATION_PLAN.md, then `git add -A` then `git commit` with a message describing the changes.
