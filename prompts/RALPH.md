...
3. When `make test` and `make lint` update @IMPLEMENTATION_PLAN.md, then `git add -A` then `git commit` with a message describing the changes (use the `commit` skill).
