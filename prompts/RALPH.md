...
3. When `make test` and `make lint` passes, run all fuzz tests with `make fuzz-slotcache`. If all good, then update @IMPLEMENTATION_PLAN.md, then `git add -A` then `git commit` with a message describing the changes.
