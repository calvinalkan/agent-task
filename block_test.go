...
func TestBlockNonexistentBlocker(t *testing.T) {
	...
	exitCode = Run(nil, &stdout, &stderr, []string{"tk", "-C", tmpDir, "block", ticketID, "nonexistent"}, nil)
...
func TestBlockSelf(t *testing.T) {
	...
	exitCode = Run(nil, &stdout, &stderr, []string{"tk", "-C", tmpDir, "block", ticketID, ticketID}, nil)
...
func TestBlockTicket(t *testing.T) {
	...
	Run(nil, &createStdout1, nil, []string{"tk", "-C", tmpDir, "create", "Ticket 1"}, nil)
	Run(nil, &createStdout2, nil, []string{"tk", "-C", tmpDir, "create", "Ticket 2"}, nil)
	...
	exitCode := Run(nil, &stdout, &stderr, []string{"tk", "-C", tmpDir, "block", ticketID1, ticketID2}, nil)
...
func TestBlockMultipleBlockers(t *testing.T) {
	...
	Run(nil, &createStdout1, nil, []string{"tk", "-C", tmpDir, "create", "Ticket 1"}, nil)
	Run(nil, &createStdout2, nil, []string{"tk", "-C", tmpDir, "create", "Ticket 2"}, nil)
	Run(nil, &createStdout3, nil, []string{"tk", "-C", tmpDir, "create", "Ticket 3"}, nil)
	...
	content := readTicket(t, ticketDir, ticketID1)
	assertContains(t, content, "blocked-by: ["+ticketID2+", "+ticketID3+"]")
}
