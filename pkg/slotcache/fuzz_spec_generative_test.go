...
func FuzzSpec_GenerativeUsage(fuzzing *testing.F) {
    fuzzing.Add([]byte{})
    fuzzing.Add([]byte("commit"))
    fuzzing.Add(make([]byte, 64))

    fuzzing.Fuzz(func(testingHandle *testing.T, fuzzBytes []byte) {
        temporaryDirectory := testingHandle.TempDir()
        cacheFilePath := filepath.Join(temporaryDirectory, "spec_gen_fuzz.slc")
        ...
        if openError != nil {
            testingHandle.Fatalf("Open failed unexpectedly: %v", openError)
        }
        defer func() {
            _ = cacheHandle.Close()
        }()
        ...
        for stepIndex := 0; stepIndex < maximumSteps && decoder.hasMoreBytes(); stepIndex++ {
            ...
            if actionByte%100 < 3 {
                ...
                if reopenError != nil {
                    ...
                    testingHandle.Fatalf("reopen failed: %v", reopenError)
                }
                writerHandle = nil
            ...
            case 2:
                ...
                if commitError == nil {
                    ...
                    if validationError := validateSlotcacheFileAgainstOptions(cacheFilePath, options); validationError != nil {
                        testingHandle.Fatalf("speccheck failed after commit: %v", validationError)
                    }
                }
...
// specFuzzDecoder is a minimal byte reader for spec fuzzing.
