// Behavioral correctness: metamorphic testing
//
// Oracle: self-consistency (semantic laws that must hold)
// Technique: deterministic seeded sequences
//
// Unlike model-comparison tests, these verify semantic invariants:
// - "last write wins" for same key
// - Scan forward/reverse are exact mirrors
// - Committed data survives Close/Reopen
// - Uncommitted writes are invisible to readers
//
// Failures here mean: "a semantic invariant was violated"
	...
	for i := range seedCount {
		...
		t.Run(fmt.Sprintf("seed=%d", seed), func(t *testing.T) {
			...
			for i := 0; i < baseOps && decoder.HasMore(); i++ {
				...
				if put, isPutOp := op.(testutil.OpPut); isPutOp {
					if mErr, isErrorResult := mRes.(testutil.ResErr); isErrorResult {
